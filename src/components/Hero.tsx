      ...
      <div className="relative z-10 flex flex-col justify-center pt-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          ...
          >
            ...
            >
              {t(`slides.${carouselSlides[currentImageIndex].section}.title`)}
            </motion.h1>
            <motion.p
              initial={{opacity: 0, y: 20}}
              animate={{opacity: 1, y: 0}}
            ...
            >
              {t(`slides.${carouselSlides[currentImageIndex].section}.subtitle`)}
            </motion.p>
            <motion.div
              initial={{opacity: 0, y: 20}}
              animate={{opacity: 1, y: 0}}
            ...
            >
              <Link href={carouselSlides[currentImageIndex].path}>
                <button className="bg-white text-blue-600 px-8 py-3 rounded-lg text-lg font-semibold hover:bg-blue-50 transition-colors inline-flex items-center">
                  {t(`slides.${carouselSlides[currentImageIndex].section}.cta`)}
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-30 flex space-x-2">
        {carouselSlides.map((_, index) => (
          <button
            ...
            aria-label={`Go to ${carouselSlides[index].section} section`}
          />
        ))}
      </div>
    </section>
